...
//			.Добавить("РезультатJSON = ОбщийКлиентСервер.ОбъектВJSON(РаботаСКоллекцией.КоллекцияЗначенийВСтруктуру(КоллекцияЗначений, Истина));")
		;
	КонецЕсли;
...
Процедура КомандаСформироватьНаСервере()
	...
	Если ТипЗнч(Результат) = Тип("ТаблицаЗначений") Тогда
		Объект.РезультатJSON = ОбщийКлиентСервер.ОбъектВJSON(РаботаСКоллекцией.ТаблицаЗначенийВСтруктуру(Результат, Истина));
	ИначеЕсли ТипЗнч(Результат) = Тип("ДеревоЗначений") Тогда
		Объект.РезультатJSON = ОбщийКлиентСервер.ОбъектВJSON(РаботаСКоллекцией.ДеревоЗначенийВСтруктуру(Результат, Истина));
	Иначе
...
Процедура ВывестиУсловияОтбора(МодельТекста, Элементы)
	Для Каждого ЭлементОтбора Из Элементы Цикл
		...
		Иначе
			МодельТекста
				.Добавить(".Отбор(""{1}"", ВидСравненияКомпоновкиДанных.{2}, ""{3}"")", ЭлементОтбора.ЛевоеЗначение, ОбщийКлиентСервер.CamelCase(ЭлементОтбора.ВидСравнения), ЭлементОтбора.ПравоеЗначение)
			;
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
