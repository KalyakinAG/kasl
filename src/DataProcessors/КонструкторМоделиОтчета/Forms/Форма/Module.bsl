...
Функция ПолучитьСтруктуруМакета()
	...
	РезультатJSON = Объект.РезультатJSON;
	Результат = Объект.Результат; 
	УстановитьБезопасныйРежим(Истина);
	Выполнить(Объект.ТекстМодели);
	Текст = Новый ТекстовыйДокумент;
	Текст.ДобавитьСтроку(ОбщийКлиентСервер.ЗначениеВJSON(МодельМакетаКомпоновки.СтруктураМакета()));
	Возврат Текст; 
КонецФункции
...
Процедура КомандаПолучитьТекстМоделиОтчетаНаСервере()
	...
	Для Каждого НаборДанных Из СхемаКомпоновкиДанных.НаборыДанных Цикл
		...
		ИначеЕсли ТипЗнч(НаборДанных) = Тип("НаборДанныхОбъединениеСхемыКомпоновкиДанных") Тогда
			Для Каждого НаборДанныхОбъединения Из НаборДанных.Элементы Цикл
				МодельТекста
					.Добавить("ТекстЗапроса%1 = """, НаборДанных.Имя)
					.ГруппировкаНачать()
						.Присоединить(СтрЗаменить(СтрЗаменить(НаборДанных.Запрос, """", """"""), Символы.ПС, Символы.ПС + "|"))
	...
	Для Каждого НаборДанных Из СхемаКомпоновкиДанных.НаборыДанных Цикл
		...
		РесурсыНабора = РаботаСМассивом.Отобрать(Ресурсы, "РаботаСМассивом.Содержит(Контекст, Элемент.Поле)", НаборДанных.Поля);
		РаботаСМассивом.Дополнить(ПоляИтогов, РесурсыНабора);
		...
		Если ЗначениеЗаполнено(РесурсыНабора) Тогда
			...
			Для Каждого Поле Из РесурсыНабора Цикл
				...
				Если Роль.Остаток Тогда
					...
					КонецЕсли;
				Иначе
					Выражение = Поле.Значение.Выражение;
					Если СтрПодобнаПоРегулярномуВыражению(Выражение, "^[a-zA-Zа-яА-Я_]\w*\(\w+\)$", Истина, Ложь) Тогда
						ОписаниеАргументов = ОтобратьЗначимыеАргументы(Аргументы);
						МодельТекста.Добавить(СтрШаблон(".%1(%2)", Лев(Выражение, СтрНайти(Выражение, "(") - 1), ОписаниеАргументов), Аргументы);
					Иначе
						Аргументы.Добавить(Выражение);// Выражение
						Аргументы.Добавить(Поле.Значение.Группировки);
						ОписаниеАргументов = ОтобратьЗначимыеАргументы(Аргументы);
						МодельТекста.Добавить(СтрШаблон(".Поле(%1)", ОписаниеАргументов), Аргументы);
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;	
			МодельТекста
	...
	ВычисляемыеРесурсы = РаботаСМассивом.Отобрать(Ресурсы, "НЕ РаботаСМассивом.Содержит(Контекст, Элемент)", ПоляИтогов);
	
	//  Вычисляемые ресурсы
	Если ЗначениеЗаполнено(ВычисляемыеРесурсы) Тогда
		Для Каждого Поле Из ВычисляемыеРесурсы Цикл
			ПолеНабораДанных = НаборДанных.Поля.Найти(Поле.Поле);
			Если ПолеНабораДанных = Неопределено Тогда
				ВычисляемоеПоле = СхемаКомпоновкиДанных.ВычисляемыеПоля.Найти(Поле.Поле);
				Если ВычисляемоеПоле = Неопределено Тогда
	...
	Если НЕ ЗначениеЗаполнено(Объект.ТипГенератора) ИЛИ Объект.ТипГенератора = "ТабличныйДокумент" Тогда
		МодельТекста
			...
			.ГруппировкаЗавершить()
			.Добавить(";")
			.Добавить("Результат.Очистить();")
			.Добавить("МодельМакетаКомпоновки.Вывести(Результат);")
		;
	Иначе
		МодельТекста
			.Добавить("МодельМакетаКомпоновки = Общий.МодельМакетаКомпоновкиДанных()")
...
//			.Добавить("РезультатJSON = ОбщийКлиентСервер.ЗначениеВJSON(РаботаСКоллекцией.КоллекцияЗначенийВСтруктуру(КоллекцияЗначений, Истина));")
		;
	КонецЕсли;
...
&НаСервере
Процедура ЗагрузитьСхему(ИмяМакета)
	Макет = РеквизитФормыВЗначение("Объект").ПолучитьМакет(ИмяМакета);
	Объект.ТекстСКД = ОбщегоНазначения.ЗначениеВСтрокуXML(Макет);
КонецПроцедуры

&НаСервере
Процедура ЗагрузитьТекст(ИмяМакета)
	Объект.ТекстМодели = РеквизитФормыВЗначение("Объект").ПолучитьМакет(ИмяМакета).ПолучитьТекст();
КонецПроцедуры
...
Процедура КомандаСформироватьНаСервере()
	...
	Если ТипЗнч(Результат) = Тип("ТаблицаЗначений") Тогда
		Объект.РезультатJSON = ОбщийКлиентСервер.ЗначениеВJSON(РаботаСКоллекцией.ТаблицаЗначенийВСтруктуру(Результат, Истина));
	ИначеЕсли ТипЗнч(Результат) = Тип("ДеревоЗначений") Тогда
		Объект.РезультатJSON = ОбщийКлиентСервер.ЗначениеВJSON(РаботаСКоллекцией.ДеревоЗначенийВСтруктуру(Результат, Истина));
	Иначе
...
Процедура ВывестиУсловияОтбора(МодельТекста, Элементы)
	Для Каждого ЭлементОтбора Из Элементы Цикл
		...
		Иначе
			МодельТекста
				.Добавить(".Отбор(""%1"", ВидСравненияКомпоновкиДанных.%3, ""%2"")", ЭлементОтбора.ЛевоеЗначение, ЭлементОтбора.ПравоеЗначение, ОбщийКлиентСервер.CamelCase(ЭлементОтбора.ВидСравнения))
			;
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
&НаКлиенте
Процедура КомандаЗагрузитьДемоВыводДереваЗначений(Команда)
	ЗагрузитьТекст("ДемоВыводДереваЗначений");
КонецПроцедуры

&НаКлиенте
Процедура КомандаЗагрузитьДемоВыводТаблицыЗначений(Команда)
	ЗагрузитьТекст("ДемоВыводТаблицыЗначений");
КонецПроцедуры
