...
Функция ИспользоватьМетаданные(_Метаданные) Экспорт
	Перем Поле;
	ПоляОсновнойТаблицы = Новый Структура;
	ИмяОсновнойТаблицы = "";
	...
	Для Каждого ОписаниеТаблицы из ТаблицыХранения Цикл
		Если ОписаниеТаблицы.Назначение <> "ИтогиСрезПоследних" И ОписаниеТаблицы.Назначение <> "Итоги" И ОписаниеТаблицы.Назначение <> "Основная" И ОписаниеТаблицы.Назначение <> "ТабличнаяЧасть" Тогда
			Продолжить;
		КонецЕсли;
		Если ОписаниеТаблицы.Назначение = "Итоги" Тогда
			ОписаниеТаблицы.ИмяТаблицы = ИмяОсновнойТаблицы + ".Остатки";
		КонецЕсли;
		Если ОписаниеТаблицы.Назначение = "ИтогиСрезПоследних" Тогда
			ОписаниеТаблицы.ИмяТаблицы = ИмяОсновнойТаблицы + ".СрезПоследних";
		КонецЕсли;
		Поля = Новый Структура;
		Если ОписаниеТаблицы.Назначение = "Итоги" ИЛИ ОписаниеТаблицы.Назначение = "ИтогиСрезПоследних" Тогда
			Для Каждого ОписаниеПоля Из ОписаниеТаблицы.Поля Цикл
				Если НЕ ПоляОсновнойТаблицы.Свойство(ОписаниеПоля.ИмяПоляХранения, Поле) Тогда
					Продолжить;
				КонецЕсли;
				Поля.Вставить(Поле.ИмяПоля, Новый Структура("ИмяПоляХранения, ИмяПоля", Поле.ИмяПоляХранения, Поле.ИмяПоля));
			КонецЦикла;
		Иначе
			Для Каждого ОписаниеПоля Из ОписаниеТаблицы.Поля Цикл
				Поле = ОписаниеПоля;
		...
		Если ОписаниеТаблицы.Назначение = "Основная" Тогда
			ИмяОсновнойТаблицы = ОписаниеТаблицы.ИмяТаблицы;
			ПоляОсновнойТаблицы = Новый Структура;
			Для Каждого Поле Из Поля Цикл
				ПоляОсновнойТаблицы.Вставить(Поле.Значение.ИмяПоляХранения, Поле.Значение);
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
	Возврат ЭтотОбъект;
...
Функция ПолучитьТекстПараметров() Экспорт
	...
	Для Каждого ЭлементПараметра Из Параметры Цикл
		...
		Если ЗначениеЗаполнено(Параметр.ТипЗначения) Тогда
			ПредставлениеЗначения = Параметр.Значение;
		...
		ИначеЕсли ТипЗначения = Тип("Дата") Тогда
			ПредставлениеЗначения = СтрШаблон("'%1'", Формат(Параметр.Значение, "ДФ=yyyy-MM-ddTHH:mm:ss"));
...
Функция Источник(ИмяТаблицыИсточника, Псевдоним = "") Экспорт
...
Функция Псевдоним(Путь)
	Позиция = СтрНайти(Путь, ".", НаправлениеПоиска.СКонца);
	Если Позиция > 0 Тогда
		Псевдоним = Прав(Путь, СтрДлина(Путь) - Позиция);
	Иначе
		Псевдоним = Путь;
	КонецЕсли;
	Возврат Псевдоним;
...
Функция ИсточникИзКоллекции(Источник)
	...
	Для Каждого СтрокаЗначений Из Таблица.Строки Цикл
		...
		Для ИндексКолонки = 0 По Колонки.ВГраница() Цикл
			...
			ТипЗначения = ВРег(Колонка.ТипЗначения);
			Значение = СтрокаЗначений[ИндексКолонки];
			Если СтрНачинаетсяС(ТипЗначения, "СТРОКА") Тогда
				Если СтрНачинаетсяС(Значение, "0x") Тогда
					Значения.Добавить(Значение);
				Иначе
					Значения.Добавить(СтрШаблон("'%1'", Значение));
				КонецЕсли;
			ИначеЕсли СтрНачинаетсяС(ТипЗначения, "ЧИСЛО") Тогда
				Значения.Добавить(Формат(Значение, "ЧРД=.; ЧН=0; ЧГ="));
			ИначеЕсли СтрНачинаетсяС(ТипЗначения, "ДАТА") Тогда
				Значения.Добавить(СтрШаблон("CAST('%1' AS DATETIME2)", Значение));
			Иначе
				Значения.Добавить(Значение);
			КонецЕсли;
		КонецЦикла;
...
Функция ПолучитьТекстЗапросаОператора(ИндексОператора) Экспорт
	...
	Если ПоляСАгрегатнойФункцией.Количество() > 0 И ПоляСАгрегатнойФункцией.Количество() <> Оператор.ВыбранныеПоля.Количество() Тогда
		...
		ПоляГруппировки = РаботаСМассивом.АТДМассив(Оператор.ВыбранныеПоля)
			.Отобрать("НЕ Элемент.СодержитАгрегатнуюФункцию И НЕ Лев(Элемент.Выражение, 1) = Символ(39)") // + если выражение является строковой константой и начинается с символа '
			.ВМассив()
		;
