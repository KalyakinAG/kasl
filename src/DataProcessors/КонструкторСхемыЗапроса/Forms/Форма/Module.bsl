...
Процедура ДобавитьТекстМодуляЗапросаПакета(ТекстМодуля, ЗапросПакета, Приставка = "", ЭтоВложенныйЗапрос = Ложь)
	...
	Для Каждого ОператорВыбрать Из ЗапросПакета.Операторы Цикл
		...
		//		ДобавитьСтрокуМодуля(ТекстМодуля, "[ОператорВыбрать].Группировка.Добавить(""[ВыражениеГруппировки]"");", Новый Структура("ВыражениеГруппировки", ОбщийКлиентСервер.ЭкранироватьТекст(ВыражениеГруппировки)), Приставка);
		//	КонецЦикла;
		//КонецЕсли;
	КонецЦикла;
