...
// Преобразует строку в массив, структуру в массив структур {Ключ, Значение}
// 
// Параметры:
//	Значение	- Строка - свойства через запятую, которые преобразуются в массив
//	            - Структура, Соответствие из Произвольный - преобразует в массив структур {Ключ, Значение}
...
Функция Массив(Значение) Экспорт
	...
	Если ТипЗначения = Тип("Структура") 
		...
		ИЛИ ТипЗначения = Тип("ФиксированноеСоответствие") Тогда
			Результат = ОтобразитьСвойства(Значение, "Ключ, Значение");
			Возврат Результат;
	КонецЕсли;
	Возврат Новый Массив;
КонецФункции
...
Функция ОпределитьПорядок(Список, СвойствоПорядка, СвойствоПодчиненныхЭлементов, Очередь, Словарь, СвойствоЗначенияСловаря, Знач Порядок = 0)
	ТипКоллекции = ТипЗнч(Список);
	ЭтоКлючЗначение = (ТипКоллекции = Тип("Соответствие") ИЛИ ТипКоллекции = Тип("Структура"));
	Для Каждого ЭлементИлиКлючЗначение Из Список Цикл
		//  Нужно проверить тип элемента. Если это подчиненные Список, то это ключ, верхнего - это сам Список
		Если ЭтоКлючЗначение Тогда
			Элемент = ЭлементИлиКлючЗначение.Значение;
		ИначеЕсли ТипЗнч(ЭлементИлиКлючЗначение) = Тип("Строка") Тогда
			Если ЗначениеЗаполнено(СвойствоЗначенияСловаря) Тогда
				Элемент = Словарь[ЭлементИлиКлючЗначение][СвойствоЗначенияСловаря];
			Иначе
				Элемент = Словарь[ЭлементИлиКлючЗначение];
			КонецЕсли;
		Иначе
		...
		//  Рекурсивный спуск
		Порядок = ОпределитьПорядок(Элемент[СвойствоПодчиненныхЭлементов], СвойствоПорядка, СвойствоПодчиненныхЭлементов, Очередь, Словарь, СвойствоЗначенияСловаря, Порядок);
		//  Контроль циклической ссылки
