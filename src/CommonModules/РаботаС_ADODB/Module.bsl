...
Функция ПараметрыСоединения() Экспорт
	...
	_ПараметрыСоединения.Вставить("ПривестиУИНКСтроке", Истина);
	_ПараметрыСоединения.Вставить("СмещениеДат2000", Истина);
...
Функция ПрочитатьНаборЗаписей(НаборЗаписей, СмещениеДат2000, ПривестиУИНКСтроке = Истина)
	Перем МассивHEX;
	...
	Для ИндексКолонки = 0 По НаборЗаписей.Fields.Count - 1 Цикл
		Поле = НаборЗаписей.Fields(ИндексКолонки);// Name, Value
		...
		ИначеЕсли Type = 72 Тогда // Указывает глобальный уникальный идентификатор (GUID) (DBTYPE_GUID)
			Если ПривестиУИНКСтроке Тогда
				ОписаниеТипа = Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(36));
				ТипКолонкиСтрокой = "СТРОКА(36)";
			Иначе
				ОписаниеТипа = Новый ОписаниеТипов("УникальныйИдентификатор");
				ТипКолонкиСтрокой = "УНИКАЛЬНЫЙИДЕНТИФИКАТОР";
			КонецЕсли;
	...
	Пока НЕ НаборЗаписей.EOF() Цикл
		...
		Для ИндексКолонки = 0 По НаборЗаписей.Fields.Count - 1 Цикл
			...
			ИначеЕсли Поле.Type = 72 Тогда
				Значение = Сред(Поле.Value, 2, 36);
				Если НЕ ПривестиУИНКСтроке Тогда
...
Функция ВыполнитьПакетЗапросов(СоединениеADODB, ТекстЗапроса, СмещениеДат2000 = Истина, ПривестиУИНКСтроке = Истина) Экспорт
	НаборЗаписей = СоединениеADODB.Execute(ТекстЗапроса);
	ПакетРезультатов = Новый Массив;
	ПакетРезультатов.Добавить(ПрочитатьНаборЗаписей(НаборЗаписей, СмещениеДат2000, ПривестиУИНКСтроке));
	Пока Истина Цикл
...
Функция ВыполнитьЗапрос(СоединениеADODB, ТекстЗапроса, СмещениеДат2000 = Истина, ПривестиУИНКСтроке = Истина) Экспорт
	ПакетРезультатов = ВыполнитьПакетЗапросов(СоединениеADODB, ТекстЗапроса, СмещениеДат2000, ПривестиУИНКСтроке);
